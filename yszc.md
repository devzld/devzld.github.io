...
（1）为了唯一辨识您的账号信息，我们会收集您的设备信息（包括设备序列号、Android ID、Mac地址等）；
（2）


聚合通知软件本身:

目的、应用场景：用于确定用户唯一标识、统计、日志相关功能的使用、用于通知黑名单拦截功能的正常使用。

范围、收集的个人信息的类型：设备Mac地址、IP地址、设备IMSI（国际移动用户识别码）、IMEI（国际移动设备识别码）、设备厂商、设备型号

方式：获取设备唯一标识权限、存储权限、地理位置权限

我们的软件还使用了一些第三方公司的SDK，他们也会收集一定的信息，我们建议您阅读该第三方的隐私政策：

（2）穿山甲SDK:

目的、应用场景：提供用户可能感兴趣的广告内容

范围、收集的个人信息的类型：设备厂商、设备型号、Mac地址、IMSI、IMEI、IP地址
...
范围、收集个人信息类型：设备信息（IMEI/Mac/android ID/IDFA/OPENUDID/GUID/SIM卡IMSI/地理位置信息）
...
您有权获取您的用户信息副本，您可以通过发送邮件到zhanglingdong@hcbztech.com获得，在技术可行的前提下，例如数据接口匹配，我们还可按您的要求，直接将您的用户信息副本传输给您指定的第三方。
